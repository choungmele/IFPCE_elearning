      ...
      <div class="container-fluid">
        <div class="row">
            ...
            <main role="main" class="col-lg-10 col-md-8 col-sm-12 ml-sm-auto px-md-4">
             ...
             <!-- resources/views/gestionnaire/communications/visualiser.blade.php -->

             <h1>{{ $communication->title }}</h1>

             @if($communication->hasMedia('communications'))
                 @foreach($communication->getMedia('communications') as $media)
                     @if($media->mime_type == 'application/pdf')
                         <embed src="{{ $media->getUrl() }}" type="application/pdf" width="100%" height="600px" />
                     @else
                         <!-- Afficher le lien de téléchargement pour les autres types de fichiers -->
                         <a href="{{ $media->getUrl() }}" download>{{ $media->file_name }}</a>
                     @endif
                 @endforeach
             @endif
