                ...
                <div class="container">
                    <div class="row align-items-start">
                      <div class="col s12">
                        ...
                        <form action="/creer/updateCours/traitement" method="POST" class="form-group">
                            @csrf
                            <input type="text" name="id" style="display:none; " value="{{ $courss->id }}">
                            
                            <div class="form-group">
                                <label for="Specialite" class="form-label">Specialites</label>
                                @foreach ($specialites as $specialite)
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="specialite{{ $specialite->id }}" name="specialite[]" value="{{ $specialite->id }}" 
                                            @foreach(optional($courss->specialites)->pluck('id') ?? [] as $selectedSpecialiteId)
                                                @if($selectedSpecialiteId === $specialite->id) 
                                                    checked 
                                                @endif
                                            @endforeach
                                        >
                                        <label class="form-check-label" for="specialite{{ $specialite->id }}">{{ $specialite->nom }}</label>
                                    </div>
                                @endforeach
                            </div>
                            
                            <div class="form-group">
                                <label for="Classe" class="form-label">Module de Rattachement</label>
                                <input type="text" class="form-control" id="Classe" name="moduleRattachement" value="{{ $courss->moduleRattachement }}">
                            </div>
                            <div class="form-group">
                                <label for="Classe" class="form-label">intitule cours</label>
                                <input type="text" class="form-control" id="Classe" name="intituleCours" value="{{ $courss->intituleCours }}">
                            </div>
                            <!--
                            <div class="form-group">
                                <label for="Classe" class="form-label">code</label>
                                <input type="text" class="form-control" id="Classe" name="code" value="{{ $courss->code }}">
                            </div>
                        -->
                            <div class="form-group">
                                <label for="Classe" class="form-label">volume horaire</label>
                                <input type="text" class="form-control" id="Classe" name="volumeHoraire" value="{{ $courss->volumeHoraire }}">
                            </div>
                            <div class="form-group">
                                <label for="Classe" class="form-label">coefficient</label>
                                <input type="text" class="form-control" id="Classe" name="coefficient" value="{{ $courss->coefficient }}">
                            </div>
                            <div class="form-group">
                                <label for="observations" class="form-label">objectif du cours</label>
                                <textarea class="form-control" id="observations" name="observations">{{ $courss->observations }}</textarea>
                            </div> 
                            <div class="form-group">
                                <label for="syllabus" class="form-label">syllabus</label>
                                <textarea class="form-control" id="syllabus" name="syllabus">{{ $courss->syllabus }}</textarea>
                            </div>
                            <button type="submit" class="btn btn-primary">modifier un cours</button>
                            <br><br>
                            <a href="/creer/listeCours" class="btn btn-danger">Revenir a la liste des cours</a>
                        </form>
