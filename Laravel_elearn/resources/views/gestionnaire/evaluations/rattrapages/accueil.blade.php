                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($rattrapages as $rattrapage)
                                <tr>
                                    <td>{{ $rattrapage->title }}</td>
                                    <td>{{ $rattrapage->publish_at }}</td>
                                    <td>
                                         <a href="{{ Storage::url($rattrapage->file_path) }}" download class="btn btn-info">Télécharger</a>
                                        <a href="{{ route('rattrapages.edit', $rattrapage->id) }}" class="btn btn-warning">Modifier Délai</a>
                                        <form action="{{ route('rattrapages.destroy', $rattrapage->id) }}" method="post" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger" onclick="return confirm('Voulez-vous vraiment supprimer cette épreuve de rattrapage ?')">Supprimer</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
