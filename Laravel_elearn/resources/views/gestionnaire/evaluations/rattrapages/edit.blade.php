      ...
      <div class="container-fluid">
        <div class="row">
            ...
            <main role="main" class="col-lg-10 col-md-8 col-sm-12 ml-sm-auto px-md-4">
                ...
                <div class="container">
                    <h1>Modifier l'épreuve de rattrapage</h1>
                    <form action="{{ route('rattrapages.edit', $rattrapage->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="title">Titre :</label>
                            <input type="text" name="title" id="title" class="form-control" value="{{ $rattrapage->title }}" required>
                        </div>
                        <div class="form-group">
                            <label for="file">Fichier :</label>
                            <input type="file" name="file" id="file" class="form-control-file">
                            @if ($rattrapage->file_path)
                                <p>Fichier actuel : <a href="{{ asset('storage/' . $rattrapage->file_path) }}" target="_blank">{{ basename($rattrapage->file_path) }}</a></p>
                            @else
                                <p>Aucun fichier actuel.</p>
                            @endif
                        </div>
                        <div class="form-group">
                            <label for="publish_at">Date de publication :</label>
                            <input type="datetime-local" name="publish_at" id="publish_at" class="form-control" value="{{ \Carbon\Carbon::parse($rattrapage->publish_at)->format('Y-m-d\TH:i') }}" required>
                        </div>
                        <br>
