...
<body>
      ...
      <div class="container-fluid">
        <div class="row">
            ...
            <main role="main" class="col-lg-10 col-md-8 col-sm-12 ml-sm-auto px-md-4">
                ...
                <!-- resources/views/sessions/apprenants-par-session.blade.php -->

                 <form action="/apprenants-par-specialite" method="get">
                    @csrf
                   <label for="specialite_id">Choisissez une specialite :</label>
                ...
                <br>
                @if(isset($apprenants))
    <h2>Apprenants de la specialite sélectionnée :</h2>
    ...
    <form action="{{ route('export.pdf.specialite') }}" method="post">
        @csrf
        ...
        <tbody>
            @foreach($specialites as $specialite)
                <tr>
                    ...
                    <td>{{ $specialite->code_specialite }}</td>
                    <td>
                        <form action="{{ route('specialites.destroy', $specialite->id) }}" method="post" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="return confirm('Voulez-vous vraiment supprimer cette spécialité ?')">Supprimer</button>
                        </form>
                    </td>
                </tr>
            @endforeach
