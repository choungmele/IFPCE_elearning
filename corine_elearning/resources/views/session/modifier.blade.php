...
@endif
        <form  action="{{ route('session.store_modif' , $sessions->id) }}" method="POST" enctype="multipart/form-data" id="formFormateur" name="formFormateur">
            @csrf
            <div class="form-group">
                <label for="nom">Nom de la session:</label>
                <input type="text" name="nom" id="nom" class="form-control" placeholder="nom du domaine de competence" value="{{ $sessions->nom }}"required>
                @if ($errors->has('nom'))
